...
const OUR_PROPS = [
  ...
  keyof SortableProps<any>,
  keyof SortableJs.Options
>[];
...
export default function Sortable<T>(props: SortableProps<T>) {
      ...
      onAdd(event) {
        ...
        const newItems = children.map(
          (v) =>
            ourProps.items.find(
              (item) =>
                (item[ourProps.idField] as string).toString() === v.dataset.id!
            ) || dragging.item
        );
  ...
  createEffect<Pick<
    SortableProps<T>,
    ArrayToUnion<typeof SORTABLE_OPTION_FIELDS>
  > | null>((prev) => {
